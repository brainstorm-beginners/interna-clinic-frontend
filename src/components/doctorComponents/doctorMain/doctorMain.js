...
const DoctorMain = ({doctorIIN, openedSection}) => {
    ...
    useEffect(() => {
        ...
        const fetchDoctorsPatientsData = async (doctorIIN) => {
            ...
            try {
                let response = await fetch(`http://localhost:8080/api/v1/doctors/${doctorIIN}/patients?page=${currentPage}`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': 'Bearer ' + accessToken
                ...
                if (!response.ok && response.status === 401) {
                    try {
                        await refresh();
                        accessToken = localStorage.getItem('accessToken');
                        response = await fetch(`http://localhost:8080/api/v1/doctors/${doctorIIN}/patients?page=${currentPage}`, {
                            method: 'GET',
                            headers: {
                                'Content-Type': 'application/json',
                                'Authorization': 'Bearer ' + accessToken
